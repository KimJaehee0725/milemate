...
모델 호출은 다음 구성으로 사용한다.
- model: gpt-5.2-codex
- runtime: OpenAI Codex SDK / Responses API
- test mode: fake SDK client

실행 전략
- `CodexClient`가 OpenAI SDK 호출을 감싼다.
- Microsoft Agent Framework graph runner는 Codex client를 injectable dependency로 받는다.
- unit test와 발표 demo는 network를 호출하지 않는 deterministic graph path를 기본으로 둔다.
...
- `app.yaml`: 앱 전역 설정, 모델, Codex SDK runtime, demo state 설정
