...
이 문서는 mock MVP를 실제 vLLM/MCP 연결 전 단계의 implementation skeleton으로 강화한 현재 기준을 정리한다.

## 현재 구현 기준

- `StageManager`는 `SessionStore` 인터페이스 뒤에서 상태를 읽고 쓴다.
- 기본 런타임과 테스트는 `InMemorySessionStore`를 사용한다.
- durable state가 필요하면 `SQLiteSessionStore`를 주입한다.
...
- vLLM 호출은 `VLLMClient` boundary에 격리되어 있고 unit test에서는 network를 호출하지 않는다.
...
| Worker A: State/Durability | `app/backend/core/stage_manager.py`, `session_store.py`, `rollback_manager.py`, `tests/test_stage_manager.py` | store boundary, SQLite, approval gate, rollback history |
...
| Worker C: Orchestrator/Services | `app/backend/core/orchestrator.py`, `app/backend/services/**`, `tests/test_orchestrator.py` | stage handler registry, injected services, final report gate |
| Worker D: Retrieval/Model | `app/backend/integrations/**`, `tests/test_retrieval_adapters.py` | provider boundary, citation normalization, vLLM request boundary |
| Worker E: Frontend/Demo | `app/frontend/**`, `data/demo_inputs/**`, frontend helper tests | scenario demo input, safe approve, rollback preset/context |
...
5. Worker D가 retrieval/model boundaries를 추가한다.
...
- `VLLMClient`는 OpenAI-compatible request payload를 만들고 호출할 수 있지만, unit test는 fake/request construction만 검증한다.
- Microsoft Agent Framework graph는 아직 연결하지 않았으며, `Orchestrator.stage_handlers` registry가 교체 지점이다.
